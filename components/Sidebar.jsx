...
const Sidebar = ({expand, setExpand}) => {
  ...
  return (
    <div className={`flex flex-col justify-between bg-black	 pt-7 transition-all
    z-50 max-md:absolute max-md:h-screen ${expand ? 'p-4 w-64' : 'md:w-20 w-0 max-md:overflow-hidden'}`}>
      <div>
        <div className={`flex ${expand ? 'flex-row gap-10' : 'flex-col items-center gap-8'}`}>
          <Image className={expand ? 'w-35' : 'w-10'} src={expand ? assets.logo_text : assets.logo_icon}
          alt=''/>

          <div onClick={() => expand ? setExpand(false) : setExpand(true)}
          className='group relative flex items-center justify-center hover:bg-gray-600/20
        ...
        <button className={`mt-8 flex items-center justify-center cursor-pointer ${expand ? 'bg-black hover:opacity-90 rounded-2xl gap-2 p-2.5 w-max' : 'group relative h-9 w-9 mx-auto hover:bg-gray-600/30 rounded-lg'}`}>
        ...
        <div className={`mt-8 text-white/50 text-sm ${expand ? 'block' : 'hidden'}`}>
      ...
      <div>
        <div className={`flex items-center cursor-pointer group relative ${expand ? 'gap-1 text-white text-sm p-2.5 border border-black rounded-lg hover:bg-white/10 cursor-pointer' : 
          'h-10 w-10 mx-auto hover:bg-gray-600/30 rounded-lg'}`}>
